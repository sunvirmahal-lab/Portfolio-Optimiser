...
`evaluate_portfolio_risk()` and `compare_portfolios()`/`compare_all_portfolios()` print the numeric results only — the guidance below (kept here rather than repeated in the output every time these run) explains what typical values mean.
